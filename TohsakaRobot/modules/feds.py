...
from TohsakaRobot import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
...
from TohsakaRobot.modules.helper_funcs.extraction import extract_user, extract_user_and_text
...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Failed to create federation! Head over to @tohsakas to notify us of the error.")
			return
	...
	if args:
		if args[0] == 'json':
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="rin_fbanned_users.json",
			...
			with BytesIO(str.encode(backups)) as output:
				...
				update.effective_message.reply_document(document=output, filename="rin_fbanned_users.csv",
